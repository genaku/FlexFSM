...
public class FSMTestIncl {
	...
	private <E extends Object> FSM<A0, E> createA0() {
		...
		(A0.TWO) {
			...
			public void handleEvent() { a1.handleEvent((String) event());
			                            log("handle2");
			                            if( event().equals("E1") )      next(A0.THREE); }
		...
		(A0.THREE) {
			...
			public void handleEvent() { a1.handleEvent((String) event());
				                        log("handle3"); 
			                            if( event().equals("E2") )      next(A0.ONE); }
	...
	private <E extends Object> FSM<A1, E> createA1() {
		...
		(A1.ONE) {
			...
			public void handleEvent() { log("handle11"); 
                                        if( event().equals("E2") )      next(A1.TWO); }
		...
		(A1.TWO) {
			...
			public void handleEvent() { log("handle12"); 
                                        if( event().equals("E2") )      next(A1.ONE); }
